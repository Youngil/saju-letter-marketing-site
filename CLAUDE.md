...
- **전 저장소 종합 버그 점검 3회차(2026-09-08, meta 저장소 `docs/audit-2026-09-08-full-sweep.md` 참고) — 이 저장소 배정 4건 전부 대응**
  - **[High] `sitemap.ts`에 ISR 재검증 주기 누락** — 2026-09-06 블로그 DB 하이브리드 전환 때 `[lang]/page.tsx`/`blog/page.tsx`/`blog/[slug]/page.tsx`는 "이게 없으면 DB에 새로 발행한 글이 다음 배포 전까지 안 보인다"는 이유로 `revalidate = 3600`을 붙였는데, 같은 DB(`getAllPostSummaries`)를 조회하는 `sitemap.ts`에는 그때 빠져 있었다 — 그 결과 새 글이 라우트 자체엔 1시간 내로 나타나도 검색엔진용 `sitemap.xml`에는 다음 코드 배포 전까지 영원히 안 올라갈 수 있었다. 같은 값(1시간)으로 맞췄다.
  - **[High] pt/vi 트랜잭션 링크 조용히 깨짐** — 이 사이트에는 두 개의 서로 다른 언어 축이 있다는 걸 이번에 재확인했다: (a) **콘텐츠 축**(홈/블로그/compare 등, `LAUNCH_CONTENT_LANGUAGES` 4개가 맞다 — 2026-09-07 결정 유지), (b) **트랜잭션/기존 데이터 접근 축**(`/compat/[token]`·`/privacy`·`/unsubscribe` — 이미 발급된 링크/구독이 언어와 무관하게 계속 동작해야 하며, 원래 `MARKETING_LANGUAGES` 6개 전부를 지원하도록 설계돼 있었다). 2026-09-07 커밋(`ff41953`)이 `[lang]/layout.tsx`의 최상위 게이트를 `isLaunchContentLanguage`(4개)로 좁히면서, (b)축 페이지들까지 전부 pt/vi에 대해 404가 나게 됐다 — 이미 발송된 웰컴 드립 메일의 pt/vi 수신거부 링크, 이미 공유된 pt/vi 궁합 링크가 조용히 깨진 상태였다. `compat/[token]/page.tsx`/`privacy/page.tsx`는 그 커밋에서 자기 자신의 게이트까지 `isLaunchContentLanguage`로 잘못 좁혀져 있었고(`unsubscribe/page.tsx`는 애초에 손대지 않아 `isMarketingLanguage`만 쓰고 있었지만 레이아웃이 먼저 막아 도달을 못 했다), 둘 다 `isMarketingLanguage`(6개)로 되돌렸다(`generateStaticParams`/`languageAlternates`도 `MARKETING_LANGUAGES`로 원복). **레이아웃 자체의 수정 방향은 "라우트별 분기"가 아니라 "합집합만 통과시키고 세부 판정은 리프 페이지에 위임"이다** — `[lang]/layout.tsx`는 자신의 동적 세그먼트(`lang`)만 받고 하위 pathname을 모르므로(App Router 레이아웃의 근본 제약), 게이트를 `isMarketingLanguage`(6개)로 완화하고, 콘텐츠 축 페이지들(홈/블로그/blog/[slug]/compare/disclaimer/lunar-new-year)이 이미 갖고 있던 자기 자신의 `isLaunchContentLanguage` 검사가 pt/vi를 계속 404 처리하도록 그대로 맡겼다. `disclaimer`는 (b)축으로 넓히지 않았다 — 확인해보니 외부(모바일 앱 등)에서 6개 언어로 이 URL을 직접 링크하는 경로가 없고(`privacy`는 모바일의 `buildPrivacyPolicyUrl(language)`가 6개 언어로 직접 링크한다는 확실한 이유가 있음), 사이트 내부에서만 쓰이는 이 페이지로의 유일한 진입점(`footer` 링크)조차 이미 콘텐츠 축 4개 언어 레이아웃 안에서만 렌더되므로 넓힐 근거가 없었다. 회귀 테스트: `vitest.config.mts`(이 저장소가 그동안 `@/` 별칭을 해석하는 vitest 설정이 없어 `src/lib/*` 상대 경로 파일만 테스트할 수 있었던 것을 처음으로 풀었다 — Next.js 자체의 dev/build 파이프라인과는 무관, `npm test`에만 영향) 신설 후 `[lang]/layout.test.ts`(7건, 6개 언어 전부 notFound 없이 통과 + 미지원 언어는 여전히 404)·`compat/[token]/page.test.ts`(7건)·`privacy/page.test.ts`(13건)·`unsubscribe/page.test.ts`(13건) 추가.
  - **[Medium] GA4가 쿠키/추적 동의 없이 항상 발화** — 완전한 CMP 대신 최소 동의 배너 + Google Consent Mode v2로 대응(사용자가 명시적으로 선택한 절충안). `GoogleAnalytics.tsx`가 gtag.js 로드 *전에* `gtag('consent','default', {...: 'denied'})`로 4개 저장소(`analytics_storage`/`ad_storage`/`ad_user_data`/`ad_personalization`)를 전부 거부 상태로 시작하고(GA4 스크립트 자체는 계속 로드 — Consent Mode는 "동의 전엔 익명화된 신호만" 방식), 이미 저장된 선택이 있으면 같은 인라인 스크립트 안에서 동기적으로 한 번 더 갱신한다(재방문자가 `ConsentBanner` 마운트를 기다리며 잠깐 `denied`로 새는 것 방지). 새 `src/components/ConsentBanner.tsx`(`'use client'`, 허용/거부 버튼) + `src/lib/analytics.ts`의 `readStoredConsent`/`storeConsent`(localStorage 키 `saju-letter-consent`, 1년 TTL — 만료되면 배너 재노출) 신설. `dict.consent`(`message`/`acceptButton`/`declineButton`) 6개 언어 전부 추가 — ⚠️ 이 문구는 AI 초안이라 `privacyPolicy.ts`와 같은 수준의 법률 전문가 검토가 필요하다(`dictionaries/types.ts`의 `consent` 필드 주석에 명시). `GA_MEASUREMENT_ID`가 없으면(로컬 개발) 추적 자체가 없으니 배너도 함께 숨긴다. 회귀 테스트: `src/lib/analytics.test.ts`(11건, `vi.stubGlobal`로 `window`/`localStorage`/`gtag`를 최소 흉내 — 이 저장소에 jsdom이 없어 실제 DOM 대신 이 방식을 택했다) — 저장 없음/1년 이내 granted·denied 유지/1년 초과 만료/깨진 JSON 흡수/SSR(`window` undefined)에서 예외 없음/`gtag` 미로드 시 예외 없음까지 커버.
  - **[Low] `compat/[token]/opengraph-image.tsx`·`[lang]/opengraph-image.tsx`가 4언어 축소 형제 수정에서 누락** — `compat/[token]/opengraph-image.tsx`는 재확인 결과 이미 `isMarketingLanguage`만 쓰고 있어(4개로 좁혀진 적 없음) 그대로 뒀다(위 (b)축 결론과 일치, 코멘트만 컨텐츠 파일 쪽에 보강). `[lang]/opengraph-image.tsx`(홈 세그먼트)는 처음엔 지시대로 `compare/opengraph-image.tsx`처럼 `isLaunchContentLanguage`까지 좁히려 했으나, **좁히면 새 회귀가 생긴다는 걸 재검증 중 발견해 좁히지 않는 것으로 결론냈다** — `privacy/page.tsx`/`disclaimer/page.tsx`가 이 세그먼트의 이미지를 폴백 OG 이미지로 URL 직접 참조(`openGraph.images: [\`${WEB_BASE_URL}/${lang}/opengraph-image\`]`)하는데, `privacy`는 바로 위 항목에서 (b)축(6개 언어)으로 되돌렸으므로 이 파일을 4개로 좁히면 pt/vi 개인정보처리방침의 OG 이미지가 깨진다(이번 점검에서 되돌린 pt/vi 접근성을 이 파일이 다시 부분적으로 깨는 셈). `isMarketingLanguage`만 유지하고, 이유를 파일 상단 주석에 남겨 실수로 다시 좁히는 시도를 막아뒀다. 회귀 테스트: `[lang]/opengraph-image.test.tsx`(6건, `ImageResponse`/satori 자체는 vitest 환경에서 실행하지 않고 그 앞단 언어 선택 로직만 dictionary 조회로 검증 — pt/vi가 'en' 폴백으로 안 떨어지는지, 각 언어가 실제로 자기 자신의 `hero.title`을 쓰는지).
  - **검증**: 타입체크·전체 테스트 스위트(95/95, 11개 파일 — 새 파일 6개 추가) 통과, `npm run build` 프로덕션 빌드 성공(App Router 라우트 검증 포함 — `/[lang]/privacy`·`/[lang]/unsubscribe`가 다시 6개 언어 정적 페이지로 생성되는 것을 빌드 로그에서 직접 확인).

- **전 저장소 종합 버그 점검 4회차(2026-09-08, 3회차 같은 날 이어서, meta 저장소 `docs/audit-2026-09-08-full-sweep-4.md` 참고) — 이 저장소 배정 2건 대응**
  - **[High] 신년운세 캠페인이 4개 언어로 축소되며 이미 발급된 pt/vi 링크가 전부 깨짐** — 3회차(위 항목)가 `compat`/`privacy`/`unsubscribe`를 "트랜잭션 축"으로 재분류해 `MARKETING_LANGUAGES`(6)로 되돌렸을 때, 신년운세 캠페인(`lunar-new-year/page.tsx`/`r/[id]/page.tsx`/`unsubscribe/page.tsx`)은 "콘텐츠 축"(4개가 맞다)으로 분류돼 손대지 않았다 — 하지만 이 판단은 신년운세 캠페인의 실제 역사를 놓친 것이었다. `git show ff41953^:src/lib/languages.ts`로 확인한 결과, 2026-09-07 이전 이 캠페인은 원래 `NON_KOREAN_LANGUAGES`(5개, en/es/pt/ja/vi — ko 미지원)를 썼는데, `ff41953`("서비스 언어 축소")이 `LAUNCH_CONTENT_LANGUAGES`(4개, ko/en/ja/es)로 바꾸면서 ko는 의도대로 추가됐지만(사용자가 "지금까지 한국은 배제해왔으니 한국도 포함한다"고 명시적으로 결정, `saju-letter-backend/CLAUDE.md` §9 확장 기능 #15 참고) 원래 있던 pt/vi가 실수로 함께 빠졌다 — `r/[id]`(신년운세 결과/공유 페이지)·`unsubscribe`(드립 수신거부) 둘 다 이미 발급된 링크가 계속 열려야 하는 트랜잭션 축인데, `src/dictionaries/pt.ts`/`vi.ts`에 `lunarNewYear` 콘텐츠가 그대로 남아있는데도(확인 완료) pt/vi 방문자가 전부 404를 만나고 있었다. **언어 집합 결정**: ko를 뺄 이유가 없으므로(위 backend 문서에 남은 명시적 의도) "ko 제외 5개로 원복" 대신 **`MARKETING_LANGUAGES`(6) 전부**로 복원해 ko+pt/vi를 모두 살렸다 — 우연히 이 두 조건(ko 포함 + pt/vi 포함)의 합집합이 정확히 6개 전체와 같다. `page.tsx`/`r/[id]/page.tsx`/`unsubscribe/page.tsx` 3개 라우트의 `generateStaticParams`/`generateMetadata`/기본 export 게이트를 전부 `isMarketingLanguage`(6)로 바꿨고, 그 아래 컴포넌트(`LunarNewYearHome.tsx`/`ReadingForm.tsx`/`OffSeasonPlaceholder.tsx`)와 API 타입(`lunarNewYearApi.ts`의 `CreateReadingInput.language`/`ReadingView.language`)에 흘러가던 `LaunchContentLanguage`(4) 타입도 전부 `MarketingLanguage`(6)로 넓혔다. **`OffSeasonPlaceholder.tsx`의 `INTL_LOCALE` 맵이 ko/en/ja/es 4개뿐이라 pt/vi 방문자가 오프시즌 화면에서 `formatDate` 호출 시 `undefined` locale로 크래시할 뻔한 걸 타입 확장 과정에서 발견해 함께 고쳤다** — `pt: 'pt-BR'`(이 저장소가 `posts.ts`에서 이미 쓰는 관례와 통일)/`vi: 'vi-VN'` 추가. **백엔드가 신년운세 캠페인의 실제 제출 가능 언어를 `getActiveServiceLanguages()`(현재 ko/en/ja/es 4개 활성, pt/vi는 `ServiceLanguage` 테이블에서 비활성)로 동적으로 제한하고 있다는 점도 확인했다** — 이는 이 사이트의 정적 라우팅 축과는 별개의, 백엔드가 소유한 런타임 설정이라 이번 수정 범위 밖으로 뒀다. pt/vi로 새로 폼을 제출하면 백엔드가 `unsupported_language`를 반환하고 `ReadingForm.tsx`는 이를 일반 에러 문구로 흡수한다(크래시 없음, 기존에도 있던 처리) — 이 항목이 고치는 건 "이미 발급된 링크가 깨지는" 문제이지, "새 제출을 pt/vi에서도 활성화"하는 별개의 백엔드 정책 결정이 아니다. **회귀 테스트 신설** — 이 경로엔 테스트가 전무했다: `page.test.ts`(9건, `generateStaticParams`가 6개 언어를 반환하는지 포함)·`r/[id]/page.test.ts`(13건, `getReading`을 목킹)·`unsubscribe/page.test.ts`(9건) — 전부 `compat/[token]/page.test.ts`와 같은 "6개 언어 전부 404 없이 통과 + 미지원 언어는 여전히 404" 패턴.
  - **[Medium] `sitemap.ts`의 privacy 항목이 3회차 수정 이후에도 여전히 4개 언어만 나열** — 3회차가 `privacy/page.tsx`의 게이트는 `MARKETING_LANGUAGES`(6)로 되돌렸지만, `sitemap.ts`의 `privacyEntries`는 여전히 `LAUNCH_CONTENT_LANGUAGES`(4)를 쓰고 있었다 — `/pt/privacy`·`/vi/privacy`가 실제로 정상 응답하는데 sitemap엔 안 실리는 상태였다(옛 주석 "2026-09-07부터... 4개로 좁혔다"도 3회차 수정 이후 사실과 어긋난 채 남아 있었다). `privacyEntries`를 `MARKETING_LANGUAGES`(6)로 고치고 주석도 현재 상태(3회차가 이미 6개로 되돌렸다는 사실)에 맞게 정정했다. **위 신년운세 항목에서 언어 집합을 6개로 확정한 뒤, `lunarNewYearEntries`도 함께 `MARKETING_LANGUAGES`로 갱신했다** — 수정 전엔 우연히 라우팅(4개)과 일치했지만, 위 항목의 라우팅 변경(4→6)에 맞춰 sitemap도 같이 바꿔야 실제 존재하는 6개 언어 페이지가 전부 색인 대상에 오른다. `disclaimerEntries`/홈(`homeEntries`)은 이번 범위 밖(여전히 의도적으로 4개, 위 3회차 항목의 (a)축 결정 그대로 유효 — disclaimer는 3회차에서 이미 "넓힐 근거 없음"으로 검토 완료됐고 이번 두 항목 모두 그 결론과 무관하다).
  - **검증**: 타입체크 클린(`npx tsc --noEmit`), 전체 테스트 스위트(136/136, 14개 파일 — 새 파일 3개·31건 추가) 통과, `npm run build` 프로덕션 빌드 성공(`/[lang]/lunar-new-year`·`/[lang]/lunar-new-year/unsubscribe`가 각각 6개 언어 정적 페이지로, `/[lang]/lunar-new-year/r/[id]`가 동적 라우트로 정상 생성되는 것을 빌드 로그에서 직접 확인 — `/[lang]/privacy`도 여전히 6개 언어 유지 확인).

- **전 저장소 종합 버그 점검 5회차(2026-09-09, meta 저장소 `docs/audit-2026-09-09-full-sweep.md` 참고) — 이 저장소 배정 1건 대응**
  - **[High] `[lang]/layout.tsx`의 로고·이용안내 링크가 언어 게이트 없이 렌더돼 pt/vi 방문자에게 404** — 3~4회차 수정으로 이 레이아웃 자체는 "콘텐츠 축(4개)·트랜잭션 축(6개)의 합집합만 통과시키고 세부 판정은 리프 페이지에 맡긴다"는 원칙으로 이미 `isMarketingLanguage`(6)로 넓어져 있었고, 같은 파일의 블로그/compare 내비 링크(3회차)도 정확히 `isLaunchContentLanguage(lang)`로 게이트돼 있었다 — 그런데 **같은 형제 수정 대상이었어야 할 다른 두 링크(헤더 로고, 푸터 "서비스 이용 안내")가 그때 게이트 없이 남아있었다**. 이 회차가 지적한 "여러 회차에 걸쳐 반복돼온 형제 코드 누락 패턴"의 새 사례.
    - **로고 링크(헤더 브랜드 마크)** — `href="/${lang}"`로 홈(`[lang]/page.tsx`)을 가리키는데, 홈은 콘텐츠 축(`LAUNCH_CONTENT_LANGUAGES`, 4개)만 서비스한다는 명시적 제품 결정(meta 저장소 CLAUDE.md §9 "1차 출시 언어는 ko/en/ja/es 4개")이 있어 pt/vi에선 이 링크를 눌러도 404가 났다. 블로그/compare 링크와 달리 이 로고는 브랜드 마크라 자체를 숨기면 안 된다고 판단해, `isLaunchContentLanguage(lang)`로 감싸되 아닌 경우엔 `<Link>` 대신 같은 시각 요소를 그대로 그리는 비클릭 `<span>`으로 대체했다(링크가 사라지는 게 아니라 눌러도 아무 일도 없는 상태 — `AppDownloadLinks`의 "출시 준비 중" 배지와 같은 원칙).
    - **푸터 "서비스 이용 안내"(disclaimer) 링크** — 대상 페이지 `disclaimer/page.tsx`가 2026-09-07 커밋에서 `LAUNCH_CONTENT_LANGUAGES`(4)로 좁혀져 있어 pt/vi에선 링크 자체(레이아웃 게이트 통과)는 눌리는데 그 다음 페이지가 404였다. `content/disclaimer.ts`를 확인한 결과 pt/vi 번역이 이미 갖춰져 있어(포팅 시점부터 6개 언어 전부 작성됨), 링크를 숨기는 대신 **페이지 게이트 자체를 `privacy/page.tsx`와 같은 `isMarketingLanguage`(6)로 되돌리는 쪽을 택했다** — disclaimer도 privacy와 마찬가지로 법적/안전 고지 문서라 1차 출시 언어 축과 무관해야 한다는, 애초 2026-09-02 신설 당시의 원칙 그대로다(`generateStaticParams`도 `MARKETING_LANGUAGES`로 원복). `sitemap.ts`의 `disclaimerEntries`도 같은 이유로 `MARKETING_LANGUAGES`(6)로 맞췄다(4회차가 privacy에서 겪은 것과 같은 "라우트는 넓혔는데 sitemap을 안 맞추면 죽은 색인 갭이 생긴다" 패턴을 이번엔 같은 커밋에서 함께 처리) — 레이아웃 링크는 이제 게이트 없이 항상 렌더해도 안전하다(주석으로 명시).
  - **검증**: 타입체크 클린(`npx tsc --noEmit`), 전체 테스트 스위트(149/149, 15개 파일 — `disclaimer/page.test.ts` 신규 13건 추가, `privacy/page.test.ts`와 동일 패턴) 통과, `npm run build` 프로덕션 빌드 성공(`/[lang]/disclaimer`가 `/[lang]/privacy`와 동일하게 6개 언어 정적 페이지로 생성되는 것을 빌드 로그에서 확인). 이 저장소에 컴포넌트 렌더링 테스트 인프라가 없어(기존 관례대로) 로고 `<span>` 폴백 자체의 전용 테스트는 추가하지 않았다 — `layout.test.ts`의 기존 "6개 언어 전부 notFound 없이 통과" 테스트가 이 조건부 분기를 포함해 실행은 계속 검증한다.

- **최종 pre-launch 감사 — 언어 전환기가 `?token=` 쿼리스트링을 버리는 버그 수정(2026-09-09, meta 저장소 `docs/audit-2026-09-09-final-prelaunch.md` "saju-letter-marketing-site" 섹션 대응)** — `LanguageSwitcher.tsx`의 `pathForLanguage`가 `usePathname()`만으로 이동할 경로를 만들고 있었는데, `usePathname()`은 쿼리스트링을 포함하지 않는다. `UnsubscribeStatus.tsx`/`lunar-new-year/UnsubscribeStatus.tsx` 둘 다 `useSearchParams().get('token')`에만 의존해 상태를 판단하는 페이지라, 이메일 링크로 `/en/unsubscribe?token=abc123`에 들어온 방문자가 헤더의 언어 전환기를 누르면 `/{newLang}/unsubscribe`로(토큰 없이) 이동해 곧바로 "찾을 수 없음" 상태를 보게 됐다 — 실제로는 수신거부가 이미 성공했을 수도 있는데 실패로 오인시키는 문제였다.
  - **두 라우트만 특별 처리하지 않고, 모든 언어 전환에 현재 쿼리스트링을 그대로 이어붙이는 일반 해법을 택했다** — 이 사이트는 언어 스위처가 `[lang]/layout.tsx` 한 곳에서만 렌더되고 페이지별로 다르게 동작할 필요가 없으며, 쿼리 파라미터가 언어 전환 후에도 유지되는 쪽이 일반적으로 유용하지 해가 되지 않는다(다른 페이지의 언어 전환 링크가 의도적으로 쿼리 파라미터를 지워야 하는 경우가 있는지 확인했지만 없었다). `src/lib/languages.ts`에 순수 함수 `buildLanguageSwitchPath(restOfPath, lang, queryString)`을 신설(기존 `availableSwitcherLanguages`처럼 로직을 컴포넌트 밖으로 뽑아 렌더링 테스트 인프라 없이도 단위 테스트 가능하게 함)하고, `LanguageSwitcher.tsx`가 `useSearchParams().toString()`으로 얻은 값을 이 함수에 넘긴다.
  - **`useSearchParams()`를 새로 쓰면서 Suspense 경계 문제가 딸려왔다** — 이 훅을 쓰는 클라이언트 컴포넌트를 Suspense로 감싸지 않으면 그 컴포넌트를 포함한 정적 페이지 전체가 정적 렌더링에서 빠지거나 빌드가 깨진다(Next.js App Router 공식 제약, `usePathname()`만 쓰던 기존 코드는 이 제약이 없었다). 이 사이트는 `generateStaticParams`로 대부분의 `[lang]/...` 페이지를 정적 생성하므로 반드시 감싸야 했다 — `LanguageSwitcher.tsx`를 `LanguageSwitcherInner`(실제 훅 로직)와 그 Suspense 래퍼(`export function LanguageSwitcher`)로 분리하고, 폴백은 열려있지 않은 드롭다운 버튼과 동일하게 보이는 `LanguageSwitcherFallback`을 둬 정적 셸에 잠깐 보이다 하이드레이션 후 실제 컴포넌트로 자연스럽게 교체되게 했다.
  - **회귀 테스트**: `languages.test.ts`에 `buildLanguageSwitchPath` 4건(쿼리 없음/단일 토큰 보존/복수 파라미터 보존/루트 경로에서도 보존) 추가.
  - **검증**: `npx tsc --noEmit` 클린, 전체 테스트 스위트(153/153, 기존 149 + 신규 4) 통과, `npm run build` 프로덕션 빌드 성공(`/[lang]/unsubscribe`/`/[lang]/lunar-new-year/unsubscribe` 포함 전 라우트가 여전히 정적 생성되는 것을 빌드 로그에서 확인 — Suspense 관련 컴파일 에러 없음). 이 저장소에 컴포넌트 렌더링 테스트 인프라가 없어(기존 관례대로) `LanguageSwitcher.tsx` 자체의 전용 렌더링 테스트는 추가하지 않았다.
