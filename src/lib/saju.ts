...
/**
 * saju-letter-mobile/src/domain/saju/calculateSaju.ts를 그대로 옮긴 것(양력 입력만 지원하는
 * 버전) — saju-letter-newyear-campaign/src/lib/saju.ts에 이은 세 번째 포팅이다. 이 사이트도
 * 사주 계산을 브라우저에서 직접 한다("이 백엔드는 계산을 하지 않는다" 원칙,
 * saju-letter-backend/CLAUDE.md §2). 백엔드로는 계산 결과(천간/지지)만 전송하고, 원본
 * 생년월일시는 절대 서버로 보내지 않는다.
 */
...
export function calculateSaju(input: SajuFormInput): SajuChart {
  ...
  const eightChar = Solar.fromYmdHms(input.year, input.month, input.day, hour, minute, 0).getLunar().getEightChar();
